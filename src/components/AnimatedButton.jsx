...
const AnimatedButton = ({ text, width }) => {
  const [hovered, setHovered] = useState(false);

  return (
    <div className="bg-white rounded-full border px-1 py-0.5 border-gray-300 inline-block">
      <button
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        className={`rounded-full flex  items-center gap-2 py-0.5  transition-colors duration-300 ${
          hovered ? "bg-secondary" : "bg-white"
        }`}
      >
        {/* Arrow wrapper */}
          ...
          <AnimatePresence mode="wait">
            {hovered ? (
              ...
              <motion.span
                ...
                className="absolute  left-0 top-0 text-[16px] font-semibold tracking-tight text-black"
              >
                {text}
              </motion.span>
            )}
